...
namespace FrameLog.Exceptions
{
    ...
    {
        private const string expectedMessageA = 
            "The underlying provider failed on EnlistTransaction.";
        private const string expectedMessageB = 
        ...
        private const string message = 
            "Attempted to open a transaction on the current connection, but there is already a transaction in progress. " +
            "If you are wrapping the call to FrameLog.SaveChanges() in an explicit transaction, and not using the TransactionScope API, " +
            "you will need to pass this transaction in using one of the overloads of FrameLog.SaveChanges(). " +
